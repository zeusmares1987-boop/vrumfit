...
function AgendaPage() {
  ...
  return (
    <AppShell>
      <PageHero icon={CalendarDays} title="Agenda" subtitle={isPersonal ? "Seus horários com alunos." : "Seus horários com o personal."}
        action={isPersonal ? (
          <button onClick={() => setOpen(true)} className="h-10 px-4 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center gap-1.5">
      ...
      {isLoading ? (
        ...
        <>
          <Section title="Próximos">
            {upcoming.length === 0 ? <p className="text-[13px] text-white/50">Nada agendado.</p> : upcoming.map((a) => (
...
function Card({ appt, isPersonal, past, onDone, onCancel, onDelete }: any) {
  ...
  const statusColor = appt.status === "concluido" ? "text-success border-success/40 bg-success/10"
    : appt.status === "cancelado" ? "text-white/50 border-white/15 bg-white/5 line-through"
    : "text-primary border-primary/40 bg-primary/10";

  return (
    <div className={`rounded-2xl border p-3 ${past ? "opacity-70" : ""} ${statusColor}`}>
      <div className="flex items-start gap-3">
        <div className="shrink-0 size-12 rounded-xl bg-black/40 border border-current grid place-items-center">
        ...
        <div className="flex-1 min-w-0 text-foreground">
          ...
          {appt.location && <p className="text-[11px] text-white/55 flex items-center gap-1 mt-0.5"><MapPin className="size-3" />{appt.location}</p>}
        </div>
      </div>
      {!past && isPersonal && appt.status === "agendado" && (
        <div className="mt-3 flex gap-2">
          <button onClick={onDone} className="flex-1 h-9 rounded-xl bg-success/15 border border-success/40 text-success text-[12px] font-bold flex items-center justify-center gap-1">
            <CheckCircle2 className="size-3.5" /> Concluído
          </button>
          <button onClick={onCancel} className="flex-1 h-9 rounded-xl bg-white/5 border border-white/15 text-white/70 text-[12px] font-bold flex items-center justify-center gap-1">
            <X className="size-3.5" /> Cancelar
          </button>
          <button onClick={onDelete} className="h-9 px-3 rounded-xl bg-destructive/15 border border-destructive/40 text-destructive">
...
function NewModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm grid place-items-end sm:place-items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-t-3xl sm:rounded-3xl bg-card border border-white/10 p-5 max-h-[90dvh] overflow-y-auto">
        ...
        <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="space-y-3">
          ...
          <button type="submit" disabled={create.isPending}
            className="w-full h-12 rounded-2xl bg-primary text-primary-foreground font-extrabold uppercase tracking-wider disabled:opacity-60">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="text-[12px] text-white/70">{label}</span>{children}</label>;
}
function Inp({ value, onChange, type = "text" }: { value: string; onChange: (v: string) => void; type?: string }) {
  return <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full h-10 rounded-xl bg-black/30 border border-white/10 px-3 text-sm focus:border-primary outline-none" />;
}
