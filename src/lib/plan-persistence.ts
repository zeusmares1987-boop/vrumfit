...
import type { DietPlan } from "@/lib/diet-engine";
import type { WeekPlan } from "@/lib/workout-engine";

type MealKind = "cafe_manha" | "lanche_manha" | "almoco" | "lanche_tarde" | "jantar" | "ceia";
const MEAL_KINDS: MealKind[] = ["cafe_manha", "lanche_manha", "almoco", "lanche_tarde", "jantar", "ceia"];

function mealKindFor(index: number, total: number): MealKind {
  if (total <= 3) return ["cafe_manha", "almoco", "jantar"][index] as MealKind;
  if (total === 4) return ["cafe_manha", "almoco", "lanche_tarde", "jantar"][index] as MealKind;
  if (total === 5) return ["cafe_manha", "lanche_manha", "almoco", "lanche_tarde", "jantar"][index] as MealKind;
  return MEAL_KINDS[Math.min(index, MEAL_KINDS.length - 1)];
}

function mapDietObjective(g: DietPlan["goal"]): "emagrecimento" | "ganho_massa" | "manutencao" {
...
export async function saveDietPlan(opts: {
  ...
  const { studentId, personalId, plan } = opts;
  const objective = mapDietObjective(plan.goal);
  const { data: diet, error } = await supabase
    .from("diets")
    .insert({
      student_id: studentId,
      personal_id: personalId ?? null,
      name: opts.name ?? `Dieta ${objective} ${plan.kcalTarget} kcal`,
      objective,
      water_liters: plan.waterMl ? plan.waterMl / 1000 : null,
      golden_tip: `TMB ${plan.bmr} kcal · GET ${plan.tdee} kcal · Meta ${plan.kcalTarget} kcal`,
      notes: `Macros: P ${plan.macros.protein}g · C ${plan.macros.carb}g · G ${plan.macros.fat}g`,
      status: "ativo",
    })
    .select("id")
    .single();
  ...
  for (let i = 0; i < total; i++) {
    ...
    const { data: m, error: me } = await supabase
      ...
      .insert({
        ...
        title: meal.title,
        time_range: meal.timeRange ?? null,
        sort_order: i,
      })
      .select("id")
      .single();
    ...
    const items = meal.items.map((it, j) => ({
      ...
      measure: it.food.unitLabel ?? null,
      sort_order: j,
    }));
    if (items.length) {
...
export async function saveWorkoutWeek(opts: {
  ...
  objective: "hipertrofia" | "emagrecimento" | "forca" | "condicionamento" | "saude" | "manutencao";
  name?: string;
  exerciseIdByName?: Map<string, string>;
}): Promise<string> {
  const { studentId, personalId, week, level, objective } = opts;
  ...
  const { data: w, error } = await supabase
    ...
    .insert({
      ...
      warmup: week.warmup ?? null,
      aerobic: week.aerobic ?? null,
      stretching: week.stretching ?? null,
      status: "ativo",
    })
    .select("id")
    .single();
  ...
  for (let i = 0; i < week.days.length; i++) {
    ...
    const { data: day, error: de } = await supabase
      .from("workout_days")
      .insert({ workout_id: w.id, label: d.label, sort_order: i })
      .select("id")
      .single();
